...
namespace ShoppingCart.DataService
{
    ...
    public class ProductHomeDataService : IProductHomeDataService
    {
        ...
        public async Task<List<Product>> GetOfferProductsAsync()
        {
            ...
            try
            {
                var uri = new UriBuilder($"{App.BaseUri}products/offer?key=zmacs3jmjprNC9gba7xf5nZZDg87NM6dLYdaTwzT");
        ...
        public async Task<List<Banner>> GetBannersAsync()
        {
            ...
            try
            {
                var uri = new UriBuilder($"{App.BaseUri}products/banner?key=zmacs3jmjprNC9gba7xf5nZZDg87NM6dLYdaTwzT");
                var response = await httpClient.GetAsync(uri.ToString());
                if (response != null && response.IsSuccessStatusCode)
                {
                    var result = response.Content.ReadAsStringAsync().Result;
