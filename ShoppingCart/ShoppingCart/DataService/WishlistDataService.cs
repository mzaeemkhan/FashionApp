...
namespace ShoppingCart.DataService
{
    ...
    public class WishlistDataService : IWishlistDataService
    {
        ...
        public async Task<Status> AddOrUpdateUserWishlist(int userId, int productId, bool isFavorite)
        {
            ...
            try
            {
                var uri = new UriBuilder(
                    $"{App.BaseUri}users/wishlist?key=zmacs3jmjprNC9gba7xf5nZZDg87NM6dLYdaTwzT&userId={userId}&productId={productId}&isFavorite={isFavorite}");
        ...
        public async Task<List<Product>> GetUserWishlistAsync(int userId)
        {
            ...
            try
            {
                var uri = new UriBuilder($"{App.BaseUri}users/wishlist?key=zmacs3jmjprNC9gba7xf5nZZDg87NM6dLYdaTwzT&userId={userId}");
