...
namespace ShoppingCart.DataService
{
    ...
    public class CategoryDataService : ICategoryDataService
    {
        ...
        public async Task<List<Category>> GetCategories()
        {
            ...
            try
            {
                var uri = new UriBuilder($"{App.BaseUri}category?key=zmacs3jmjprNC9gba7xf5nZZDg87NM6dLYdaTwzT");
        ...
        public async Task<List<Category>> GetSubCategories(int categoryId)
        {
            ...
            try
            {
                var uri = new UriBuilder($"{App.BaseUri}category?key=zmacs3jmjprNC9gba7xf5nZZDg87NM6dLYdaTwzT&categoryId={categoryId}");
                var response = await httpClient.GetAsync(uri.ToString());
                if (response != null && response.IsSuccessStatusCode)
                {
                    var result = response.Content.ReadAsStringAsync().Result;
